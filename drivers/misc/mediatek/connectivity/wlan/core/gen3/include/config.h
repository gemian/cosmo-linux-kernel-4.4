...
/* should align the value to block size boundary for block mode transaction */
#if CFG_SDIO_TX_AGG || CFG_TX_BUFFER_IS_SCATTER_LIST
#define CFG_COALESCING_BUFFER_SIZE          ((NIC_TX_PAGE_SIZE * CFG_MAX_TX_PAGE_COUNT \
						+ MY_SDIO_BLOCK_SIZE - 1) / MY_SDIO_BLOCK_SIZE * MY_SDIO_BLOCK_SIZE)
#else
...
#define CFG_RX_COALESCING_BUFFER_SIZE       (((CFG_NUM_OF_RX0_HIF_DESC  + 1) * CFG_RX_MAX_PKT_SIZE \
						+ MY_SDIO_BLOCK_SIZE - 1) / MY_SDIO_BLOCK_SIZE * MY_SDIO_BLOCK_SIZE)
#else
...
#define CFG_SUPPORT_GSCN	0	/* GSCN not ready disabled here */
#else
...
/*------------------------------------------------------------------------------
 * Flags of driver support PCC feature
 *------------------------------------------------------------------------------
 */

#define CFG_SUPPORT_PCC			1

/*------------------------------------------------------------------------------
 * Flags of driver support driver abort scan feature
 *------------------------------------------------------------------------------
 */
