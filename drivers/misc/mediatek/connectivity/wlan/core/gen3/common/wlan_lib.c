	...
	do {
...
#if CFG_ENABLE_FW_DOWNLOAD
		if (!kalFirmwareImageMapping(prAdapter->prGlueInfo, &prFwBuffer, &u4FwSize)) {
...
#if defined(MT6631) && defined(CONFIG_OF)
		if (prAdapter->prGlueInfo->rHifInfo.Dev) {
			if (of_property_read_u32_index(prAdapter->prGlueInfo->rHifInfo.Dev->of_node,
			    ...
			    of_property_read_u32_index(prAdapter->prGlueInfo->rHifInfo.Dev->of_node,
		...
		prFwHead = (P_FIRMWARE_DIVIDED_DOWNLOAD_T)prFwBuffer;

		if (prFwHead->u4Signature == MTK_WIFI_SIGNATURE &&
		    prFwHead->u4CRC == wlanCRC32((PUINT_8)prFwBuffer + u4CRCOffset,
						 u4FwSize - u4CRCOffset)) {
			/* 3b. engage divided firmware downloading */
			u4Status = wlanImageDividDownload(prAdapter, prFwBuffer);
		} else
#endif
		u4Status = wlanImageFullDownload(prAdapter, prFwBuffer, u4FwLoadAddr, u4FwSize);
		...
		while (1) {
			...
			if (u4Value & WCIR_WLAN_READY) {
				...
				DBGLOG(INIT, ERROR, "Waiting for Ready bit timeout, Device to Host MailBox 0x%x\n",
			      ...
			      IN UINT_32 index)
{
	...
	do {
		if (wlanImageSectionConfig(prAdapter,
			...
			DBGLOG(INIT, ERROR, "Firmware download configuration failed! idx %u, dst 0x%08x, len %u\n",
			       index,
			       prFwHead->arSection[index].u4DestAddr,
			       prFwHead->arSection[index].u4Length);
		...
		for (j = 0; j < prFwHead->arSection[index].u4Length; j += CMD_PKT_SIZE_FOR_IMAGE) {
			...
			if (wlanImageSectionDownload(prAdapter,
						     ...
						     WLAN_STATUS_SUCCESS) {
				DBGLOG(INIT, ERROR,
				       "Firmware scatter download failed! idx %u, dst 0x%08x, len %u, offset %u\n",
				       index,
				       prFwHead->arSection[index].u4DestAddr,
				       prFwHead->arSection[index].u4Length,
				       j);
...
wlanImageDividDownload(IN P_ADAPTER_T prAdapter, IN PVOID pvFwImageMapFile)
{
#define ENTRY_DIRECTLY_TO_CHIP 2

	UINT_32 i;
	P_FIRMWARE_DIVIDED_DOWNLOAD_T prFwHead = (P_FIRMWARE_DIVIDED_DOWNLOAD_T) pvFwImageMapFile;
	WLAN_STATUS u4Status = WLAN_STATUS_SUCCESS;
#if defined(MT6631)
	...
	/* DLM + ILM */
	for (i = 0; i < prFwHead->u4NumOfEntries && i < ENTRY_DIRECTLY_TO_CHIP; i++) {
...
#if defined(MT6631)
/* WIFI using TOP 512KB, but the last 32KB is reserved for BT */
#define WIFI_EMI_AVAILABLE_MEM_SIZE   (480 * 1024)
	...
	for (i = ENTRY_DIRECTLY_TO_CHIP; i < prFwHead->u4NumOfEntries; i++) {
		UINT_32 u4DestAddr = prFwHead->arSection[i].u4DestAddr & 0xfffff;
		UINT_32 u4Length = prFwHead->arSection[i].u4Length;

		if (u4DestAddr + u4Length > WIFI_EMI_MEM_SIZE) {
			DBGLOG(INIT, ERROR, "FW section length out of bound! idx %u, dst 0x%08x, len %u\n",
			       i, u4DestAddr, u4Length);
			u4Status = WLAN_STATUS_FAILURE;
			break;
		}
		DBGLOG(INIT, TRACE,
		       "ConsysEmiPhyBase 0x%lx, WifiEmiBaseAddr %p, idx %u, dst 0x%08x, len %u\n",
		       gConEmiPhyBase, prWifiEmiBaseAddr, i, u4DestAddr, u4Length);

		/* TODO, EMI download only if reboot */
		kalMemCopy(prWifiEmiBaseAddr + u4DestAddr,
...
WLAN_STATUS wlanPowerOffInt(IN P_ADAPTER_T prAdapter)
{
	...
	for (u4Loop = 0; u4Loop < CFG_MCU_POWER_OFF_POLLING_CNT; u4Loop++) {
		...
		HAL_MCR_RD(prAdapter, MCR_WCIR, &u4Value);
		DBGLOG(INIT, INFO, "INT FeedBack: 0x%x, MCR_WCIR: 0x%x\n", u4Feedback, u4Value);
...
#if defined(MT6631)
					if (prAdapter->prGlueInfo->rHifInfo.InfraRegBaseAddr)
						DBGLOG(INIT, INFO,
						"count %d, INFRA_TOPAXI_PROTECTEN: 0x%08x, INFRA_TOPAXI_PROTECTEN_STA1: 0x%08x\n",
						i,
						CONNSYS_REG_READ(prAdapter->prGlueInfo->rHifInfo.InfraRegBaseAddr,
								 INFRA_TOPAXI_PROTECTEN),
						CONNSYS_REG_READ(prAdapter->prGlueInfo->rHifInfo.InfraRegBaseAddr,
								 INFRA_TOPAXI_PROTECTEN_STA1));
#endif
					HAL_MCR_RD(prAdapter, MCR_WHLPCR, &u4RegValueNew);
					DBGLOG(INIT, INFO,
					       "Recorded WHLPCR: 0x%08x, Current WHLPCR: 0x%08x, fgIsFwOwn: %d\n",
					       u4RegValue, u4RegValueNew, prAdapter->fgIsFwOwn);
					if (wlanPowerOffInt(prAdapter) == WLAN_STATUS_SUCCESS)
						break;
#if defined(MT6631)
					if (prAdapter->prGlueInfo->rHifInfo.HifRegBaseAddr)
						DBGLOG(INIT, INFO,
						"count %d, HIF_DBGCR02: 0x%08x, HIF_DBGCR08: 0x%08x\n",
						i,
						CONNSYS_REG_READ(prAdapter->prGlueInfo->rHifInfo.HifRegBaseAddr,
								 CONN_HIF_DBGCR02),
						CONNSYS_REG_READ(prAdapter->prGlueInfo->rHifInfo.HifRegBaseAddr,
								 CONN_HIF_DBGCR08));
#endif
...
VOID wlanClearTxCommandQueue(IN P_ADAPTER_T prAdapter)
{
	...
	while (prQueueEntry) {
		...
		nicTxReleaseTCResource(prAdapter,
					nicTxGetCmdResourceType(prCmdInfo),
					nicTxGetCmdPageCount(prCmdInfo));
		...
		if (rTxPacketInfo.u2Flag) {
			if (rTxPacketInfo.u2Flag & BIT(ENUM_PKT_1X)) {
				...
				if (secIsProtected1xFrame(prAdapter, prStaRec)) {
					if (prStaRec->ucBssIndex <= HW_BSSID_NUM)
...
WLAN_STATUS wlanQueryNicCapability(IN P_ADAPTER_T prAdapter)
{
	...
	prAdapter->rVerInfo.u4FwOwnVersionExtend =
		(prEventNicCapability->aucFwVerExtend[0] << 24)
		| (prEventNicCapability->aucFwVerExtend[1] << 16)
		| (prEventNicCapability->aucFwVerExtend[2] << 8)
		| (prEventNicCapability->aucFwVerExtend[3]);
...
#if CFG_SUPPORT_PCC
	for (u2Cnt = 64; u2Cnt < 96; u2Cnt++)
		DBGLOG(INIT, TRACE,
			"Dump NVRAM:[0x%x~0x%x]: 0x%x-0x%x-0x%x-0x%x-0x%x-0x%x-0x%x-0x%x\n",
			u2Cnt*8, u2Cnt*8+7,
			prRegInfo->aucNvram[u2Cnt*8],
			prRegInfo->aucNvram[u2Cnt*8+1],
			prRegInfo->aucNvram[u2Cnt*8+2],
			prRegInfo->aucNvram[u2Cnt*8+3],
			prRegInfo->aucNvram[u2Cnt*8+4],
			prRegInfo->aucNvram[u2Cnt*8+5],
			prRegInfo->aucNvram[u2Cnt*8+6],
			prRegInfo->aucNvram[u2Cnt*8+7]);
#endif

	kalMemCopy(&rCmdNvramSettings.rNvramSettings,
		   &prRegInfo->prNvramSettings->u2Part1OwnVersion, sizeof(WIFI_CFG_PARAM_STRUCT));
	ASSERT(sizeof(WIFI_CFG_PARAM_STRUCT) == EXTEND_NVRAM_SIZE);
	wlanSendSetQueryCmd(prAdapter,
...
VOID wlanDumpBssStatistics(IN P_ADAPTER_T prAdapter, UINT_8 ucBssIdx)
{
	...
	if (!prBssInfo || !IS_BSS_ACTIVE(prBssInfo)) {
		DBGLOG(SW4, WARN, "Invalid BssInfo index[%u] or inactive, skip dump!\n", ucBssIdx);
		return;
	}
	...
	for (ucIdx = 0; ucIdx < CFG_NUM_OF_STA_RECORD; ucIdx++) {
		...
		if (!prStaRec || prStaRec->ucBssIndex != ucBssIdx)
			continue;
		for (eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++) {
...
wlanoidQueryStaStatistics(IN P_ADAPTER_T prAdapter,
			  IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	return wlanQueryStaStatistics(prAdapter, pvQueryBuffer, u4QueryBufferLen, pu4QueryInfoLen, TRUE);
}
...
VOID wlanCfgSetDebugLevel(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = 0; i < WLAN_CFG_SET_DEBUG_LEVEL_LEN_MAX; i++) {
		...
		if (u4DbgIdx == 0xFFFFFFFF) {
			...
			DBGLOG(INIT, INFO,
			       "Set DBG module[%u] log level to [0x%x]!", u4DbgIdx, u4DbgMask);
		}
	}
...
#if 0 /* Currently these code are useless */
		...
		IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	if (rTxDoneStatus)
		...
		DBGLOG(TX, TRACE, "DNS PKT TX DONE WIDX:PID[%u:%u] Status[%u] SeqNo[%u]\n",
			prMsduInfo->ucWlanIndex, prMsduInfo->ucPID, rTxDoneStatus, prMsduInfo->ucTxSeqNum);
