	...
	do {
		...
		case MAC_FRAME_PROBE_RSP:
			...
			if ((!nicTxIsMgmtResourceEnough(prAdapter)) || (prBssInfo->fgIsNetAbsent)) {
				DBGLOG(P2P, INFO, "Drop Tx probe response due to resource issue or net absence\n");
				fgDrop = TRUE;
...
#if CFG_SUPPORT_HOTSPOT_WPS_MANAGER
		else if (kalP2PCmpBlackList(prAdapter->prGlueInfo, prAuthFrame->aucSrcAddr)) {
			/* Hotspot Blacklist */
			return FALSE;
		}
#endif
	...
	do {
		...
		case CATEGORY_PUBLIC_ACTION:
			...
			if ((prScanInfo->eCurrentState == SCAN_STATE_SCANNING) &&
			    (prActPubVenFrame->ucPubSubType == P2P_GO_NEG_REQ)) {
				/* It will take more time to request p2p negotiation channel when scan on-going */
				DBGLOG(P2P, INFO, "Abort on-going scan when receive negotiation request\n");
				if (prScanInfo->rScanParam.ucBssIndex == prAdapter->prAisBssInfo->ucBssIndex)
					aisFsmStateAbort_SCAN(prAdapter);
				else
					p2pDevFsmRunEventAbort(prAdapter, prAdapter->rWifiVar.prP2pDevFsmInfo);
			}
	...
	case ELEM_ID_VENDOR:
		{
			...
			if (rsnParseCheckForWFAInfoElem(prAdapter, pucIEBuf, &ucOuiType, &u2SubTypeVersion)) {
				...
				if (ucOuiType == VENDOR_OUI_TYPE_P2P) {
					 ...
					 */
					P_GL_P2P_INFO_T prGlP2pInfo = (P_GL_P2P_INFO_T)prAdapter->prGlueInfo->prP2PInfo;

					if ((*fgIsP2PIE) == FALSE)
						prGlP2pInfo->u2WSCIELen[1] = 0;

					kalMemCopy(prGlP2pInfo->aucWSCIE[1] + prGlP2pInfo->u2WSCIELen[1],
								pucIEBuf, IE_SIZE(pucIEBuf));
					prGlP2pInfo->u2WSCIELen[1] += IE_SIZE(pucIEBuf);
					DBGLOG(P2P, TRACE,
					       "P2P IE is found in probe resp(supp). Len %u\n",
					       prGlP2pInfo->u2WSCIELen[1]);
