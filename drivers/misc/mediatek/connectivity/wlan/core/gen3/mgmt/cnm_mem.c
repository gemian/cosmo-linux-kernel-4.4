...
#ifdef LINUX
#if CFG_DBG_MGT_BUF
	pvMemory = (PVOID) kalMemAlloc(u4Length + sizeof(struct MEM_TRACK), PHY_MEM_TYPE);
	if (pvMemory) {
...
cnmPeerUpdate(P_ADAPTER_T prAdapter, PVOID pvSetBuffer, UINT_32 u4SetBufferLen, PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(CMD_PEER_ADD_T);
	...
	/* ++HT capability */
	for (i = 0; i < SUP_MCS_RX_BITMASK_OCTET_NUM; i++) {
		if (prCmd->rHtCap.rMCS.arRxMask[i])
			break;
	}
	if (i < SUP_MCS_RX_BITMASK_OCTET_NUM) {
