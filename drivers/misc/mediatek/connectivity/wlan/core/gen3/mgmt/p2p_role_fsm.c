...
UINT_8 p2pRoleFsmInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		cnmTimerInitTimer(prAdapter,
				  ...
				  (PFN_MGMT_TIMEOUT_FUNC) p2pRoleFsmGetStaStatistics, (ULONG) prP2pRoleFsmInfo);
...
VOID p2pRoleFsmUninit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		cnmTimerStopTimer(prAdapter, &prP2pRoleFsmInfo->rP2pRoleFsmGetStatisticsTimer);
			       ...
			       IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		...
		if (prStaRec->eAuthAssocState == AAA_STATE_SEND_AUTH2 ||
			...
			DBGLOG(P2P, INFO, "Do not delete STA_REC when auth is in progress\n");
			break;
		}
...
{
	UINT_32 u4BufLen;
	P_PARAM_GET_STA_STATISTICS prP2pQueryStaStatistics;
	P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo = (P_P2P_ROLE_FSM_INFO_T) ulParamPtr;
	P_BSS_INFO_T prP2pBssInfo = (P_BSS_INFO_T) NULL;
	...
	prP2pQueryStaStatistics = prAdapter->rWifiVar.prP2pQueryStaStatistics;
	if (!prP2pQueryStaStatistics) {
		DBGLOG(P2P, ERROR, "prP2pQueryStaStatistics=NULL\n");
		return;
	}
	...
	if (prP2pBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
		...
		if (prP2pBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE)
			COPY_MAC_ADDR(prP2pQueryStaStatistics->aucMacAddr,
			...
			LINK_FOR_EACH_ENTRY(prCurrStaRec, prClientList, rLinkEntry, STA_RECORD_T) {
				if (prCurrStaRec) {
					...
					break; /* break for LINK_FOR_EACH_ENTRY */
				}
			}
			if (!prCurrStaRec)
				return;
		...
		prP2pQueryStaStatistics->ucReadClear = TRUE;
		wlanQueryStaStatistics(prAdapter, prP2pQueryStaStatistics,
					  sizeof(PARAM_GET_STA_STA_STATISTICS), &u4BufLen, FALSE);
	}

	cnmTimerStartTimer(prAdapter, &(prP2pRoleFsmInfo->rP2pRoleFsmGetStatisticsTimer),
