...
P_MSDU_INFO_T qmEnqueueTxPackets(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfoListHead)
{
	...
	do {
		...
		if (!fgDropPacket) {
			...
			if (prCurrentMsduInfo->ucDhcpArpFlag && !prAdapter->fgEnCtiaMode)
...
VOID qmProcessPktWithReordering(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, OUT P_QUE_T prReturnedQue)
{
	...
	if (!prReorderQueParm || !(prReorderQueParm->fgIsValid)) { /* VHT single AMPDU */
		DBGLOG(QM, TRACE, "AMPDU from STA[%d] TID[%d] without BA agreement, no-reordering\n",
...
VOID qmInsertNoNeedWaitPkt(IN P_SW_RFB_T prSwRfb, IN ENUM_NO_NEED_WATIT_DROP_REASON_T eDropReason)
{
	...
	if (!prNoNeedWaitPkt) {
		DBGLOG(QM, ERROR, "qmInsertNoNeedWaitPkt alloc error SSN:[%d], DropReason:(%d)\n",
			prSwRfb->u2SSN, eDropReason);
		return;
	}
...
VOID qmHandleRxArpPackets(P_ADAPTER_T prAdapter, P_SW_RFB_T prSwRfb)
{
	...
	if (prAdapter->prAisBssInfo && prAdapter->prAisBssInfo->prStaRecOfAP &&
	    EQUAL_MAC_ADDR(&(pucData[ETH_TYPE_LEN_OFFSET + 10]), /* source hardware address */
