...
BOOLEAN nicTxReleaseTCResource(IN P_ADAPTER_T prAdapter, IN UINT_8 ucTC, IN UINT_16 u2ReleasePageCnt)
{
	...
	if (ucTC >= TC_NUM)
		return bStatus;
	if (u2ReleasePageCnt > 0) {
		/* Return free Tc page count */
		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);

		/* Real page counter */
		prTcqStatus->au2FreePageCount[ucTC] += u2ReleasePageCnt;

		/* Buffer counter. For development only */
		/* Convert page count to buffer count */
		u4BufferCountToBeFreed = (prTcqStatus->au2FreePageCount[ucTC] / NIC_TX_MAX_PAGE_PER_FRAME);
		prTcqStatus->au2FreeBufferCount[ucTC] = u4BufferCountToBeFreed;

		DBGLOG(TX, TRACE,
		       "Release: TC%u ReturnPageCnt[%hu] FreePageCnt[%hu] FreeBufferCnt[%hu]\n",
			ucTC, u2ReleasePageCnt, prTcqStatus->au2FreePageCount[ucTC],
			prTcqStatus->au2FreeBufferCount[ucTC]);
#if (CFG_SUPPORT_TRACE_TC4 == 1)
		if (ucTC == TC4_INDEX)
			wlanTraceReleaseTcRes(prAdapter, u2ReleasePageCnt,
				prTcqStatus->au2FreePageCount[ucTC]);
#endif

		prQM->au4QmTcResourceBackCounter[ucTC] += u2ReleasePageCnt;
		KAL_RELEASE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_RESOURCE);
		bStatus = TRUE;
	}
		 ...
		 IN UINT_8 ucTxDescLength, IN BOOLEAN fgIsTemplate, OUT PUINT_8 prTxDescBuffer)
{
	...
	if (prMsduInfo->pfTxDoneHandler) {
		UINT_8 ucWlanIndex = prMsduInfo->ucWlanIndex;

		if (prMsduInfo->ucPacketType == TX_PACKET_TYPE_DATA)
			ucWlanIndex |= BIT(7);
		prMsduInfo->ucPID = nicTxAssignPID(prAdapter, ucWlanIndex);
...
WLAN_STATUS nicTxMsduQueue(IN P_ADAPTER_T prAdapter, UINT_8 ucPortIdx, P_QUE_T prQue)
{
	...
	while (prMsduInfo) {
		...
		prTXD = (P_HW_MAC_TX_DESC_T)(pucOutputBuf + u4TotalLength);
#if CFG_SUPPORT_MULTITHREAD
		nicTxCopyDesc(prAdapter, (PUINT_8)prTXD,
			      prMsduInfo->aucTxDescBuffer, &ucTxDescSize);
#else
		nicTxFillDesc(prAdapter, prMsduInfo, (PUINT_8)prTXD, &ucTxDescSize);
#endif
		...
		if (prMsduInfo->eSrc == TX_PACKET_MGMT) {
			...
			if (prMsduInfo->pfTxDoneHandler) {
				...
				DBGLOG(TX, TRACE, "u2CookieLen is %u\n", prMsduInfo->u2CookieLen);
...
WLAN_STATUS nicTxCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN UINT_8 ucTC)
{
	...
	if (prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME) {
...
#if CFG_SUPPORT_MULTITHREAD
		nicTxCopyDesc(prAdapter, (PUINT_8)prTxDesc, prMsduInfo->aucTxDescBuffer, &ucTxDescLength);
#else
		nicTxComposeSecurityFrameDesc(prAdapter, prCmdInfo, (PUINT_8)prTxDesc, &ucTxDescLength);
#endif
		...
		DBGLOG(TX, INFO,
		       ...
		       HAL_MAC_TX_DESC_GET_WLAN_INDEX(prTxDesc),
		       prMsduInfo->ucPID, prCmdInfo->ucStaRecIndex,
		       ucTxDescLength + prCmdInfo->u2InfoBufLen,
		       HAL_MAC_TX_DESC_IS_PROTECTION(prTxDesc),
		       prMsduInfo->pfTxDoneHandler ? TRUE : FALSE,
		       prMsduInfo->ucTxSeqNum,
		       GLUE_GET_PKT_FLAG(prNativePacket),
		       prTxDesc->u2FixedRate,
		       prTxDesc->ucAntID,
		       prTxDesc->ucPowerOffset);
		...
		if (prMsduInfo->pfTxDoneHandler) {
			...
			if (prMsduInfo->ucPID > 0) {
				...
				if (prMsduInfo->ucPID > 0) {
					...
					DBGLOG(TX, WARN,
					       "EAPOL: Give up req TX done since no slot, handler %p, EAPOL type %d\n",
					       prMsduInfo->pfTxDoneHandler, prMsduInfo->ucEapolKeyType);
					nicTxReturnMsduInfo(prAdapter, prMsduInfo);
				}
			}
		} else {
		...
		nicTxCopyDesc(prAdapter, (PUINT_8)prTxDesc, prMsduInfo->aucTxDescBuffer, &ucTxDescLength);
#else
		nicTxFillDesc(prAdapter, prMsduInfo, (PUINT_8)prTxDesc, &ucTxDescLength);
#endif
		...
		DBGLOG(TX, INFO,
		       "TX MGMT Frame: SUBTYPE[%x] BSS[%d] WIDX:PID[%d:%d] SEQ[%d] STA[%d] LEN[%d] RSP[%d]\n",
		       (UINT_16)(prMgmtHeader->u2FrameCtrl & MASK_FC_SUBTYPE) >> OFFSET_OF_FC_SUBTYPE,
		...
		if (prMsduInfo->pfTxDoneHandler) {
			...
			if (prMsduInfo->ucPID > 0) {
				...
				if (prMsduInfo->ucPID > 0) {
					...
					DBGLOG(TX, WARN,
					       "MGMT: Give up req TX done since no slot, handler %p, SEQ:%d\n",
					       prMsduInfo->pfTxDoneHandler, prMsduInfo->ucTxSeqNum);
					cnmMgtPktFree(prAdapter, prMsduInfo);
				}
			}
		} else {
...
VOID nicTxProcessTxDoneEvent(IN P_ADAPTER_T prAdapter, IN P_WIFI_EVENT_T prEvent)
{
	...
	if (prTxDone->ucWlanIndex >= WTBL_SIZE) {
		DBGLOG(TX, ERROR, "WlanIndex in TX done event is wrong, value %d\n", prTxDone->ucWlanIndex);
		return;
	}
...
UINT_8 nicTxAssignPID(IN P_ADAPTER_T prAdapter, IN UINT_8 ucWlanIndex)
{
	...
	ucWlanIndex &= ~BIT(7);
	if (ucWlanIndex >= WTBL_SIZE) {
		DBGLOG(TX, ERROR, "Wrong wlan index %d\n", ucWlanIndex);
		return 0;
	}
	KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TXING_MGMT_LIST);
		...
		*/
		if (pu8PidPool[NIC_TX_DESC_DRIVER_PID_MAX/__BITS_PER_LONG]
			...
			for (ucRetval = NIC_TX_DESC_DRIVER_PID_MIN;
			     ...
			     ucZeroBits <= NIC_TX_DESC_PID_RESERVED_FOR_MGMT; ucRetval++) {
				if (!test_bit(ucRetval, pu8PidPool))
					ucZeroBits++;
			}
			ucRetval = 0;
	...
	if (fgCanAssign) {
		ucRetval = find_next_zero_bit(pu8PidPool, NIC_TX_DESC_DRIVER_PID_MAX + 1, NIC_TX_DESC_DRIVER_PID_MIN);
		if (ucRetval <= NIC_TX_DESC_DRIVER_PID_MAX)
			set_bit(ucRetval, pu8PidPool);
		else
			ucRetval = 0;
	}
	KAL_RELEASE_SPIN_LOCK(prAdapter, SPIN_LOCK_TXING_MGMT_LIST);
