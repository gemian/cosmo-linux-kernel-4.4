...
static struct proc_dir_entry *main2_cam_proc_entry = NULL;
#define MAIN_CAM_PROC_NAME	"AEON_CAMERA0"
#define SUB_CAM_PROC_NAME	"AEON_CAMERA1"
#define MAIN2_CAM_PROC_NAME	"AEON_CAMERA2"
static char imgsensorNameMain[256] = {0};
static char imgsensorNameSub[256] = {0};
static char imgsensorNameMain2[256] = {0};
...
static ssize_t main2_cam_proc_oem_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
...
static const struct file_operations main2_cam_proc_fops =
{
	.read = main2_cam_proc_oem_read
};
/*******************************************************************************
* Profiling
********************************************************************************/
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {	/* not implement this feature ID */
		...
		printk(" Sensor found ID = 0x%x,psensor_name(%s)\n", sensorID, psensor_inst->psensor_list->name);
		if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
			strncpy(imgsensorNameMain, psensor_inst->psensor_list->name, strlen(psensor_inst->psensor_list->name));
		else if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_SUB)
			strncpy(imgsensorNameSub, psensor_inst->psensor_list->name, strlen(psensor_inst->psensor_list->name));
		else if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN2)
			strncpy(imgsensorNameMain2, psensor_inst->psensor_list->name, strlen(psensor_inst->psensor_list->name));
		snprintf(mtk_ccm_name,sizeof(mtk_ccm_name),"%s CAM[%d]:%s;",mtk_ccm_name,psensor->inst.sensor_idx,psensor_inst->psensor_list->name);
		err = ERROR_NONE;
	}
...
static int imgsensor_probe(struct platform_device *pplatform_device)
{
	...
	main2_cam_proc_entry = proc_create(MAIN2_CAM_PROC_NAME, 0777, NULL, &main2_cam_proc_fops);
	if (NULL == main2_cam_proc_entry)
	{
		printk("proc_create %s failed\n", MAIN2_CAM_PROC_NAME);
	}	
	return 0;
