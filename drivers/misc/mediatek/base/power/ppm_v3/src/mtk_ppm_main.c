					...
					unsigned int freq, unsigned int relation)
{
	...
	if (cluster_id >= NR_PPM_CLUSTERS) {
		ppm_err("ppmv3 @%s: Invalid cluster id %d\n", __func__, cluster_id);
		return 0;
	}

	if (!ppm_main_info.cluster_info[cluster_id].dvfs_tbl) {
		ppm_ver("ppmv3 @%s: DVFS table of cluster %d is not exist!\n", __func__, cluster_id);
			...
			struct ppm_client_limit *c_limit, struct ppm_cluster_limit *p_limit)
{
	...
	ppm_ver("ppmv3 Result --> (%d)(%d)(%d)(%d) (%d)(%d)(%d)(%d)\n",
		c_limit->min_cpufreq_idx, c_limit->max_cpufreq_idx, c_limit->min_cpu_core,
		c_limit->max_cpu_core, c_limit->has_advise_freq, c_limit->advise_cpufreq_idx,
		c_limit->has_advise_core, c_limit->advise_cpu_core);
...
static void ppm_main_calc_new_limit(void)
{
	...
	list_for_each_entry(pos, &ppm_main_info.policy_list, link) {
		...
		if (pos->is_enabled && pos->is_activated && pos->is_limit_updated) {
			...
			for_each_ppm_clusters(i) {
				ppm_ver("ppmv3 @%s: applying policy %s cluster %d limit...\n", __func__, pos->name, i);
	...
	for (i = 0; i < c_req->cluster_num; i++) {
		...
		ppm_ver("ppmv3 Final Result: [%d] --> (%d)(%d)(%d)(%d) (%d)(%d)(%d)(%d)\n",
			i,
	...
	if (is_all_cluster_zero) {
		...
		ppm_err("ppmv3 all cluster max core limit are 0, dump all active policy data...\n");
		list_for_each_entry(pos, &ppm_main_info.policy_list, link) {
			ppm_lock(&pos->lock);
			if (pos->is_activated) {
				ppm_info("ppmv3 [%d] %s: perf_idx = %d, pwr_bdgt = %d\n",
						pos->policy, pos->name, pos->req.perf_idx, pos->req.power_budget);
				for_each_ppm_clusters(i) {
					ppm_info("ppmv3 cluster %d: (%d)(%d)(%d)(%d)\n", i,
	...
	unsigned int root_cluster, char *msg)
{
	...
	if (is_in_game) {
		...
		log_cnt = 1;
		if (filter_cnt) {
			ppm_info("ppmv3 Shrink %d PPM logs from last %lld ms!\n", filter_cnt, delta1);
			filter_cnt = 0;
		}
	...
	if (!filter_log)
		ppm_info("ppmv3 (0x%x)(%d)(%d)%s\n", policy_mask, min_power_budget, root_cluster, msg);
	else
		ppm_ver("ppmv3 (0x%x)(%d)(%d)%s\n", policy_mask, min_power_budget, root_cluster, msg);
...
int mt_ppm_main(void)
{
	...
	if (memcmp(c_req->cpu_limit, last_req->cpu_limit,
		...
		/* print debug message */
		ptr += snprintf(ptr, LOG_BUF_SIZE, "ppmv3 (%*pbl)", cpumask_pr_args(c_req->online_core));
		for (i = 0; i < c_req->cluster_num; i++) {
...

		{
			...
			if (notify_dvfs && !notify_hps) {
				...
				ppm_dbg(TIME_PROFILE, "ppmv3 Done! notify dvfs only! time = %lld us\n", delta);
				goto nofity_end;
				...
				ppm_dbg(TIME_PROFILE, "ppmv3 Done! notify hps only! time = %lld us\n", delta);
				goto nofity_end;
			}
		}
...
static int ppm_main_suspend(struct device *dev)
{
	...
	ppm_ver("ppmv3 %s: suspend callback in\n", __func__);
...
static int ppm_main_resume(struct device *dev)
{
	...
	ppm_ver("ppmv3 %s: resume callback in\n", __func__);
...
static int ppm_main_data_init(void)
{
	...
	if (!ppm_main_info.cluster_info) {
		ppm_err("ppmv3 @%s: fail to allocate memory for cluster_info!\n", __func__);
		ret = -ENOMEM;
		goto out;
	}
	for_each_ppm_clusters(i) {
		...
		ppm_info("ppmv3 @%s: ppm cluster %d -> core_num = %d, cpu_id = %d\n",
				__func__,
	...
	if (!ppm_main_info.client_req.cpu_limit) {
		ppm_err("ppmv3 @%s: fail to allocate memory client_req!\n", __func__);
		ret = -ENOMEM;
	...
	if (!ppm_main_info.last_req.cpu_limit) {
		ppm_err("ppmv3 @%s: fail to allocate memory for last_req!\n", __func__);
		ret = -ENOMEM;
