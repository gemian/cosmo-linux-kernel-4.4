				       ...
				       int dat[BMI160_ACC_AXES_NUM])
{
	...
	printk("OLDOFF:(%+3d %+3d %+3d): (%+3d %+3d %+3d)/(%+3d %+3d %+3d)\n",
	...
	printk("NEWOFF:(%+3d %+3d %+3d): (%+3d %+3d %+3d)/(%+3d %+3d %+3d)\n",
...
static int BMI160_ACC_CheckDeviceID(struct bmi160_accelgyro_data *obj)
{
	...
	case SENSOR_CHIP_ID_BMI_C3:
		printk("check chip id %d successfully.\n", databuf[0]);
		break;
	default:
		printk("check chip id %d %d failed.\n", databuf[0],
			databuf[1]);
		err = -1;
		break;
	}
				   ...
				   bool enable)
{
	...
	mdelay(1);
	mutex_unlock(&obj->accel_lock);
	printk("set power mode enable = %d ok!\n", enable);
	return 0;
}
...
static int BMI160_ACC_SetOSR4(struct bmi160_accelgyro_data *obj)
{
	...
	printk("[%s] acc_bmp = %d, acc_us = %d ok.\n", __func__, bandwidth,
		accel_undersampling_parameter);
	mutex_unlock(&obj->accel_lock);
	return err;
}
...
static int BMI160_ACC_SetIntEnable(struct bmi160_accelgyro_data *obj, u8 enable)
{
	...
	mutex_unlock(&obj->accel_lock);
	printk("bmi160 set interrupt enable = %d ok.\n", enable);
	return 0;
}
					     ...
					     const char *buf, size_t count)
{
	...
	if (err)
		return err;
	printk("store_acc_op_mode_value = %d .\n", (int)data);
						...
						const char *buf, size_t count)
{
	...
	printk("FIFO fifo_data_sel %d, A_en:%d, G_en:%d, M_en:%d\n",
						 ...
						 const char *buf, size_t count)
{
	...
	printk("set fifo watermark = %d ok.", (int)fifo_watermark);
	return count;
}
				 ...
				 int dat[BMG_AXES_NUM])
{
	...
	printk("UPDATE: add mapped data(%+3d %+3d %+3d)\n", dat[BMG_AXIS_X],
...
static int bmg_set_datarate(struct bmi160_accelgyro_data *obj, int datarate)
{
	...
	if (datarate == obj->gyro_datarate) {
		printk("set new data rate = %d, old data rate = %d\n",
			    ...
			    int64_t maxBatchReportLatencyNs)
{
	...
	printk("bmi160 acc set delay = (%d) ok.\n", value);
	return 0;
}
		      ...
		      void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			...
			value = *(int *)buff_in;
			printk("sensor enable/disable command: %s\n",
...
static int bmi160_gyro_enable_nodata(int en)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (err == 0) {
			printk("bmi160_gyro_SetPowerMode ok.\n");
			break;
		}
	}
	if (err < 0)
		printk("bmi160_gyro_SetPowerMode fail!\n");
...
{
	printk("don't support bmg_factory_get_raw_data!\n");
	return 0;
}
...
static int bmi160_accelgyro_spi_probe(struct spi_device *spi)
{
	...
	printk("enter bmi160_accelgyro_spi_probe");
	...
	err = request_irq(obj->accel_IRQ, bmi_irq_handler, IRQF_TRIGGER_RISING,
			  "bmi160", obj);
	if (err)
		pr_err("could not request accel_IRQ 1\n");
#else

	err = bmi160acc_setup_eint(spi);
	if (err)
		pr_err("could not request accel_IRQ 2\n");
	...
	printk(">>gyro_register_control_path\n");
...
static int bmi160_accelgyro_local_init(void)
{
	printk(">>>>-bmi160_accelgyro_local_init.\n");
