...
#define PFX "sil9022"
#define PK_DBG(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)
#define PK_ERR(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)

bool Sii9024A_HDCP_supported = false;//true;	/* if the chip is 9024A, you can support HDCP by set this variable to 1.If the chip is 9022A, it means noting. */
...
#define sil9022_DRVNAME "sii902xA"

/* static struct mxc_lcd_platform_data *Sii902xA_plat_data; */
static struct i2c_client * sil9022_I2Cclient = NULL;
static struct platform_driver *sil9022_pdev = NULL;
static const struct i2c_device_id sil9022_i2c_id[] = {{"sii9022_hdmi", 0}, {"siiedid", 0} };

#ifdef CONFIG_OF
static const struct of_device_id sil9022_of_match[] = {
	{.compatible = "mediatek,sii9022_hdmi"},
	{.compatible = "mediatek,siiedid"},	
	{},
};
#endif

struct pinctrl *sil9022ctrl = NULL;
struct pinctrl_state *sil9022_rst_high = NULL;
struct pinctrl_state *sil9022_rst_low = NULL;
//struct pinctrl_state *sil9022_1v2_en_high = NULL;
//struct pinctrl_state *sil9022_1v2_en_low = NULL;
struct pinctrl_state *sil9022_eint_as_int = NULL;
struct pinctrl_state *sil9022_dpi_gpio = NULL;
struct pinctrl_state *sil9022_dpi_func1 = NULL;
struct pinctrl_state *sil9022_i2s_gpio = NULL;
struct pinctrl_state *sil9022_i2s_func1 = NULL;
struct pinctrl_state *hdmi_hotplug_det_as_int = NULL;



int sil9022_gpio_init(struct platform_device *pdev)
{
	int ret = 0;

	sil9022ctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(sil9022ctrl)) {
		dev_err(&pdev->dev, "Cannot find camera pinctrl!");
		ret = PTR_ERR(sil9022ctrl);
	}
	
	sil9022_rst_high = pinctrl_lookup_state(sil9022ctrl, "sil9022_rst_high");
	if (IS_ERR(sil9022_rst_high)) {
		ret = PTR_ERR(sil9022_rst_high);
		PK_DBG("%s : pinctrl err, sil9022_rst_high\n", __func__);
	}
	
	sil9022_rst_low = pinctrl_lookup_state(sil9022ctrl, "sil9022_rst_low");
	if (IS_ERR(sil9022_rst_low)) {
		ret = PTR_ERR(sil9022_rst_low);
		PK_DBG("%s : pinctrl err, sil9022_rst_low\n", __func__);
	}
#if 0
	sil9022_1v2_en_high = pinctrl_lookup_state(sil9022ctrl, "sil9022_1v2_en_high");
	if (IS_ERR(sil9022_1v2_en_high)) {
		ret = PTR_ERR(sil9022_1v2_en_high);
		PK_DBG("%s : pinctrl err, sil9022_1v2_en_high\n", __func__);
	}

	sil9022_1v2_en_low = pinctrl_lookup_state(sil9022ctrl, "sil9022_1v2_en_low");
	if (IS_ERR(sil9022_1v2_en_low)) {
		ret = PTR_ERR(sil9022_1v2_en_low);
		PK_DBG("%s : pinctrl err, sil9022_1v2_en_low\n", __func__);
	}
#endif	
	sil9022_dpi_gpio = pinctrl_lookup_state(sil9022ctrl, "sil9022_dpi_gpio");
	if (IS_ERR(sil9022_dpi_gpio)) {
		ret = PTR_ERR(sil9022_dpi_gpio);
		PK_DBG("%s : pinctrl err, sil9022_dpi_gpio\n", __func__);
	}

	sil9022_dpi_func1 = pinctrl_lookup_state(sil9022ctrl, "sil9022_dpi_func1");
	if (IS_ERR(sil9022_dpi_func1)) {
		ret = PTR_ERR(sil9022_dpi_func1);
		PK_DBG("%s : pinctrl err, sil9022_dpi_func1\n", __func__);
	}

	sil9022_i2s_gpio = pinctrl_lookup_state(sil9022ctrl, "sil9022_i2s_gpio");
	if (IS_ERR(sil9022_i2s_gpio)) {
		ret = PTR_ERR(sil9022_i2s_gpio);
		PK_DBG("%s : pinctrl err, sil9022_i2s_gpio\n", __func__);
	}

	sil9022_i2s_func1 = pinctrl_lookup_state(sil9022ctrl, "sil9022_i2s_func1");
	if (IS_ERR(sil9022_i2s_func1)) {
		ret = PTR_ERR(sil9022_i2s_func1);
		PK_DBG("%s : pinctrl err, sil9022_i2s_func1\n", __func__);
	}
	
	hdmi_hotplug_det_as_int = pinctrl_lookup_state(sil9022ctrl, "hdmi_hotplug_det_as_int");
	if (IS_ERR(hdmi_hotplug_det_as_int)) {
		ret = PTR_ERR(hdmi_hotplug_det_as_int);
		PK_DBG("%s : pinctrl err, hdmi_hotplug_det_as_int\n", __func__);
	}

	return 0;
}
int sil9022_set_reset(int reset)
{
	PK_DBG("sil9022_set_reset: %d\n", reset);
	
	if (reset)
		pinctrl_select_state(sil9022ctrl, sil9022_rst_high);
	else
		pinctrl_select_state(sil9022ctrl, sil9022_rst_low);
	return 0;
}
#if 0
/* 1.2v for sil9022 */
int sil9022_1v2_onoff(int onoff)
{
	PK_DBG("sil9022_1v2_onoff: %d\n", onoff);
	
	if (onoff)
		pinctrl_select_state(sil9022ctrl, sil9022_1v2_en_high);
	else
		pinctrl_select_state(sil9022ctrl, sil9022_1v2_en_low);
	return 0;
}
#endif
int sil9022_set_gpio(int onoff)
{
	PK_DBG("sil9022_set_gpio: %d\n", onoff);
	
	if (onoff)
		pinctrl_select_state(sil9022ctrl, sil9022_dpi_gpio);
	else
		pinctrl_select_state(sil9022ctrl, sil9022_dpi_func1);
	return 0;
}
int sil9022_set_iis_gpio(int onoff)
{
	PK_DBG("sil9022_set_iis_gpio: %d\n", onoff);
	
	if (onoff)
		pinctrl_select_state(sil9022ctrl, sil9022_i2s_gpio);
	else
		pinctrl_select_state(sil9022ctrl, sil9022_i2s_func1);
	return 0;
}
...
static s32 i2c_smbus_write_byte_nostop(struct i2c_client *client, unsigned char RegOffset, unsigned char value)
{
			     ...
			     byte Length)
{
	...
	i2c_smbus_write_byte_nostop(siiSegEDID, Offset, Segment);
...
void WriteByteTPI(byte RegOffset, byte Data)
{
	...
	//printk("[%s]zhaolong hdmi write RegOffset=0x%x,Data=0x%x\n", __func__, RegOffset, Data);
}
...
void ReadBlockTPI(byte TPI_Offset, word NBytes, byte *pData)
{
	...
	for (i = TPI_Offset; i < (NBytes + TPI_Offset); i++) {
		pData[i - TPI_Offset] = i2c_smbus_read_byte_data(sil9022_I2Cclient, i);
	}
	return;
}
...
void WriteBlockTPI(byte TPI_Offset, word NBytes, byte *pData)
{
	int i;
	for (i = TPI_Offset; i < (NBytes + TPI_Offset); i++) {
		i2c_smbus_write_byte_data(sil9022_I2Cclient, i, pData[i - TPI_Offset]);
	}
...
g_sys.hdmiCableConnected,InterruptStatus,g_sys.dsRxPoweredUp,hdmi_9024_hpd_bak);
		...
		/* Check rx power */
		printk("===zhaolong===InterruptStatus=0x%x, g_sys.dsRxPoweredUp=0x%x\n", InterruptStatus, g_sys.dsRxPoweredUp);
		#if 0
...
*******************************************************/
int32_t HDMI_I2C_READ(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
...
*******************************************************/
int32_t HDMI_I2C_WRITE(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
...
#endif
static irqreturn_t sil9024_irq_handler(int32_t irq, void *dev_id)
{
...
static int hdmi_irq_registration(void)
{
	...
	sil9022_eint_as_int = pinctrl_lookup_state(sil9022ctrl, "sil9022_eint_as_int");
	if (IS_ERR(sil9022_eint_as_int)) {
		ret = PTR_ERR(sil9022_eint_as_int);
		printk("%s : pinctrl err, sil9022_eint_as_int\n", __func__);
	}

	node = of_find_compatible_node(NULL, NULL, "mediatek,sii9022-eint");
	if (node) {
		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);
		
		pinctrl_select_state(sil9022ctrl, sil9022_eint_as_int);

		hdmi_irq = irq_of_parse_and_map(node, 0);
		
		ret = request_irq(hdmi_irq , sil9024_irq_handler, IRQF_TRIGGER_NONE	,"EINT_HDMI_HPD-eint",NULL);
		if (ret > 0) {
			ret = -1;
	...
	printk("irq:%d, debounce:%d-%d:ret=%d\n", hdmi_irq, ints[0], ints[1],ret);
...
static int sil9022_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 	int ret;
	//byte InterruptStatus;
	//---check i2c func.---
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("i2c_check_functionality failed. (no I2C_FUNC_I2C)\n");
		ret = -ENODEV;
		goto err_check_functionality_failed;
	}


	if (match_id(&sil9022_i2c_id[1], client)) {	
		siiEDID = client;
		printk("attached sil9022A hmdi_EDID: %s, siiEDID=%p\n "
			"into i2c adapter successfully\n", id->name, siiEDID);
	} else if (match_id(&sil9022_i2c_id[0], client)) {
		
	sil9022_I2Cclient = client;
	PK_DBG("sil9022_i2c_probe: sil9022_I2Cclient=%p\n", sil9022_I2Cclient);


	aeon_gpio_set("sil9022_hdmi_pwren1");
	//sil9022_1v2_onoff(1);
	HDMI_reset();
...
#endif	
	hdmi_irq_registration();


		//InterruptStatus = ReadByteTPI(0x3D);
	    //printk("===WYS==[hdmi]hdmiCableConnected=%x\n",InterruptStatus);	
	} else {		
		printk("sil9022A invalid i2c adapter: can not found dev_id matched\n");
		return -EIO;
	}
...
static int sil9022_i2c_remove(struct i2c_client *client)
{
	return 0;
}


struct i2c_driver sil9022_i2c_driver = {
    .probe = sil9022_i2c_probe,
    .remove = sil9022_i2c_remove,
    .driver = {
    .name = sil9022_DRVNAME,
    .owner = THIS_MODULE,

#ifdef CONFIG_OF
    .of_match_table = sil9022_of_match,
#endif
    },
    .id_table = sil9022_i2c_id,
};

static int sil9022_probe(struct platform_device *pdev)
{
	printk("%s\n", __func__);
	sil9022_gpio_init(pdev);	
	return i2c_add_driver(&sil9022_i2c_driver);
}

static int sil9022_remove(struct platform_device *pdev)
{
    i2c_del_driver(&sil9022_i2c_driver);
    return 0;
}

static struct platform_driver sil9022_platform_driver = {
	.probe	  = sil9022_probe,
...
static int __init sil9022_init(void)
{
	...
	printk("%s\n", __func__);
	sil9022_pdev = &sil9022_platform_driver;
	if (platform_driver_register(&sil9022_platform_driver))
		printk("failed to register sil9022 driver already exist\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("ti sil9022 Driver");
MODULE_AUTHOR("sun deli <sun.deli@mediatek.com>");
