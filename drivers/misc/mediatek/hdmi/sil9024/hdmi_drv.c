...
static void hdmi_drv_get_params(struct HDMI_PARAMS *params)
{
	...
	input_resolution = HDMI_VIDEO_1920x1080p_60Hz;//HDMI_VIDEO_1920x1080p_60Hz;
	...
	params->cabletype 				= MHL_CABLE;//HDMI_CABLE;
...
static int hdmi_drv_audio_enable(bool enable)  
{
	...
	printk("===wys==[EXTD]Set_I2S_Pin, enable = %d\n", enable);
	Set_I2S_Pin(flag);
...
void hdmi_drv_power_off(void)
{
	...
	if (ucHdmi_isr_en) {
		  ...
		  //sil9022_1v2_onoff(0);
		  printk("hdmi_drv_power_off 2 keyboardlight_flag=%d\n",keyboardlight_flag);
