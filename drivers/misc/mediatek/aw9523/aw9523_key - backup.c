...
int aw9523_key_setup_eint(void)
{
	...
	if (aw9523_key->irq_node) {
#if 0		
		of_property_read_u32_array(aw9523_key->irq_node, "interrupts", ints, ARRAY_SIZE(ints));
		of_property_read_u32_array(aw9523_key->irq_node, "debounce", ints1, ARRAY_SIZE(ints1));
		aw9523_eint_gpio = of_get_named_gpio(aw9523_key->irq_node, "aw9523_eint-gpio", 0); 
		AW9523_LOG("%s interrupts ints[0] = %d, ints[1] = %d!! aw9523_eint_gpio = %d\n", __func__, ints[0], ints[1],aw9523_eint_gpio);
		AW9523_LOG("%s debounce ints1[0] = %d, ints1[1] = %d!!\n", __func__, ints1[0], ints1[1]);
		gpio_set_debounce(ints[0], ints1[0]);
		...
		///////////  GPIO 12
		aw9523_eint_gpio = of_get_named_gpio(aw9523_key->irq_node, "aw9523_eint-gpio", 0);
		if (aw9523_eint_gpio < 0) {
			printk("wgx>>>>>>  %s[%d] get aw9523_eint_gpio fail!\n",__func__,__LINE__);
		}
		AW9523_LOG("aw9523_eint_gpio = %d\n", aw9523_eint_gpio);
		ret = gpio_request(aw9523_eint_gpio, "aw9523_eint-gpio");
		if (ret)
			printk("wgx>>>>>>  %s[%d] gpio_request aw9523_eint_gpio fail, ret = %d \n",__func__,__LINE__,ret);

		gpio_direction_input(aw9523_eint_gpio); 
		aw9523_eint_test = __gpio_get_value(aw9523_eint_gpio);	
		AW9523_LOG(" input------ aw9523_eint_test = %d   [%d]\n",aw9523_eint_test,__LINE__);
		aw9523_key->irq = gpio_to_irq(aw9523_eint_gpio);		
		/////////////
#endif				
		...
		if (request_irq(aw9523_key->irq, aw9523_key_eint_func, IRQ_TYPE_EDGE_FALLING, "aw9523-eint", NULL)) {
