...
extern char* read_emmc_part_number(void);
#define EMMC_PROC_NAME "AEON_EMMC"
static struct proc_dir_entry *emmc_proc_entry = NULL;

#define EFUSE_FLAG     "AEON_EFUSE_FLAG"
static struct proc_dir_entry *efuse_flag_proc_entry = NULL;
static char *efuse_write_flag = NULL;
...
static ssize_t emmc_proc_oem_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
       char *page = NULL;
    char *ptr = NULL;
       int len, err = -1;
...
static const struct file_operations emmc_proc_fops = {
    .read = emmc_proc_oem_read
};

//zhaolong get efuse write flag 2017.5.10
static ssize_t efuse_proc_oem_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    char *page = NULL;
    char *ptr = NULL;
    int len, err = -1;
...
static const struct file_operations efuse_write_proc_fops = { 
    .read = efuse_proc_oem_read
};
/* sanford.lin end on 20160308 */
		...
		if (plcm->params->type == LCM_TYPE_DBI
			...
			/* sanford.lin add on 20160308 for get driver information */
			 aeon_lcm_drv = lcm_drv;
			  ...
			  emmc_proc_entry = proc_create(EMMC_PROC_NAME, 0777, NULL, &emmc_proc_fops);
					if (NULL == emmc_proc_entry)
					{
							printk("proc_create %s failed\n", EMMC_PROC_NAME);
					} 
					
				//zhaolong get efuse write flag 2017.5.10
				efuse_flag_proc_entry = proc_create(EFUSE_FLAG, 0777, NULL, &efuse_write_proc_fops);
				if (NULL == efuse_flag_proc_entry)
				{
				printk("proc_create %s failed\n", EFUSE_FLAG);
				}						
			 /* sanford.lin end on 20160308 */
