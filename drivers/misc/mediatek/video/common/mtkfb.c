...
#if 1
static char emmc_part_number[30];
struct tag_emmcpartnum {
       u32 size;
       u32 tag;
       char emmcpartnum[1];
};

char* read_emmc_part_number(void)
{
       return emmc_part_number;
}
EXPORT_SYMBOL_GPL(read_emmc_part_number);
#endif 
...
#if 1 
	  struct tag_emmcpartnum *emmcpartnum_tag = NULL;
	
	  emmcpartnum_tag = (struct tag_emmcpartnum *)of_get_flat_dt_prop(node, "atag,emmcpartnum", (int *)&size);
	  if (emmcpartnum_tag) {
			memset((void *)emmc_part_number, 0, sizeof(emmc_part_number));
			strcpy((char *)emmc_part_number,emmcpartnum_tag->emmcpartnum);
			emmc_part_number[strlen(emmcpartnum_tag->emmcpartnum)] = '\0';
	  }
