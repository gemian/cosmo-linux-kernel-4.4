...
#define CHARGING_PROC_NAME        "AEON_CHARGING"
static struct proc_dir_entry *charging_proc_entry;
static char *charging_name = NULL;
#endif
/* qzs end on 2018620 */
...
/* qzs add on 2018620 for get driver information */
#ifdef AEON_DEVICE_PROC_MANAGER
static ssize_t charging_proc_oem_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
			    ...
			    const struct i2c_device_id *id)
{
	...
	/* qzs add on 2018620 for get driver information */
	#ifdef AEON_DEVICE_PROC_MANAGER
	/*if((chip_id & 0xF0) == 0xa0)
		charging_name = "RT5081A";
	else if((chip_id & 0xF0) == 0x80)
		charging_name = "RT5081";
	else*/
		charging_name = "MT6371";

	charging_proc_entry = proc_create(CHARGING_PROC_NAME, 0777, NULL, &charging_proc_fops);
	if (NULL == charging_proc_entry)
	{
