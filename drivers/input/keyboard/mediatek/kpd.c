...
#define AEON_STM32_KEY
#ifdef AEON_STM32_KEY
extern int aeon_gpio_set(const char *name);
static u8 kpd_stm32_key_state = !0;
#define KPD_STM32_KEY_EINT 7
static unsigned int stm32_key_irqnr;
static unsigned int stm32_keygpiopin, stm32_keydebounce;
static unsigned int stm32_key_eint_type;

static struct wakeup_source stm32_key_suspend_lock;	/* For suspend usage */
static struct workqueue_struct *stm32_keyWorkqueue = NULL;
static struct work_struct stm32_key_fcover_work;
#endif

#define AEON_FINGER_KEY
#ifdef AEON_FINGER_KEY
static u8 kpd_finger_key_state = !0;
#define KPD_FINGER_KEY_EINT 7
static unsigned int finger_key_irqnr;
static unsigned int finger_keygpiopin, finger_keydebounce;
static unsigned int finger_key_eint_type;
...
#define STM32_DL_FW_PROC_NAME        "AEON_STM32_DL_FW"
static struct proc_dir_entry *stm32_dl_proc_entry;
static ssize_t  STM32_DL_FW_write(struct file *file, const char *buffer, size_t count,loff_t *data)
 {
         char Buf[4];
         int stat,ret=0;
         printk("STM32_DL_FW_write\n");
         if (copy_from_user(Buf, buffer, 4)){
                  return -EFAULT;
         }
         ret = sscanf(Buf,"%d",&stat);
         printk("STM32_DL_FW_write stat=%d\n",stat);
         if(stat == 1){

                 aeon_gpio_set("aeon_stm32_download_fw_high");
         }
         else if(stat == 0){

                 aeon_gpio_set("aeon_stm32_download_fw_low");
         }
         return count;
 }
 static const struct file_operations stm32_dl_proc_fops = {
...
static ssize_t  WAKE_STM32_write(struct file *file, const char *buffer, size_t count,loff_t *data)
 {
         char Buf[4];
         int stat,ret=0;
         printk("WAKE_STM32_write\n");
         if (copy_from_user(Buf, buffer, 4)){
                  return -EFAULT;
         }
         ret = sscanf(Buf,"%d",&stat);
         printk("WAKE_STM32_write stat=%d\n",stat);
         if(stat == 1){
 ...
 static const struct file_operations wake_stm32_proc_fops = {
         //.read  = WAKE_STM32_read,
         .write = WAKE_STM32_write
 };

 #define RESET_STM32_PROC_NAME        "AEON_RESET_STM32"
static struct proc_dir_entry *reset_stm32_entry;
static ssize_t  RESET_STM32_write(struct file *file, const char *buffer, size_t count,loff_t *data)
 {
         char Buf[4];
         int stat,ret=0;
         printk("RESET_STM32_write\n");
         if (copy_from_user(Buf, buffer, 4)){
                  return -EFAULT;
         }
         ret = sscanf(Buf,"%d",&stat);
         printk("RESET_STM32_write stat=%d\n",stat);
         if(stat == 1){
 ...
 static const struct file_operations reset_stm32_proc_fops = {
         //.read  = RESET_STM32_read,
         .write = RESET_STM32_write
 };
 #define AEON_CHARGE_STATUS_SUPPORT
    ...
    ptr = page; 
       ...
       if(aeon_charging_enable){
		   if(hdmi_plug_in_flag == 1){
			  ptr += sprintf(ptr, "left_charging_and_hdmi\n");//
		   }else if(hdmi_plug_in_flag==0 && aeon_otg_enable ==3){
			  ptr += sprintf(ptr, "left_charging_and_right_otg\n");//?
		   }else if(hdmi_plug_in_flag==0 && aeon_otg_enable ==2){
			  ptr += sprintf(ptr, "right_charging_and_left_otg\n");//?
			  ...
			  ptr += sprintf(ptr, "left_or_right_charging\n");	//
               ...
               ptr += sprintf(ptr, "hdmi_only\n");//
       else if(aeon_charging_enable == 0 && hdmi_plug_in_flag == 0 && aeon_otg_enable ==2)
               ptr += sprintf(ptr, "left_otg_only\n");//
       else if(aeon_charging_enable == 0 && hdmi_plug_in_flag == 0 && aeon_otg_enable ==3)
...
static ssize_t  CHARGE_STATUS_write(struct file *file, const char *buffer, size_t count,loff_t *data)
 {
         char Buf[4];
         int stat,ret=0;
         printk("CHARGE_STATUS_write\n");
         if (copy_from_user(Buf, buffer, 4)){
                  return -EFAULT;
         }
         ret = sscanf(Buf,"%d",&stat);
         printk("CHARGE_STATUS_write stat=%d\n",stat);
         if(stat == 1){
         }
         else if(stat == 0){
...
static const struct file_operations charge_status_proc_fops = {
		.read  = CHARGE_STATUS_read,
		.write = CHARGE_STATUS_write
};
#endif
...
#ifdef AEON_STM32_KEY
static void kpd_stm32_key_work_handler(struct work_struct * work)
{
	//u8 old_state = kpd_stm32_key_state;
	bool pressed;
...
EXPORT_SYMBOL(right_otg_in_report_key);
EXPORT_SYMBOL(right_otg_out_report_key);
EXPORT_SYMBOL(left_otg_in_report_key);
EXPORT_SYMBOL(left_otg_out_report_key);
/*----------------------------------------------------------------------------*/
/* for autotest */
#if KPD_AUTOTEST
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	__set_bit(KEY_F17, kpd_input_dev->keybit);
...
#ifdef AEON_STM32_KEY
	mt_eint_register();
	stm32_keyWorkqueue = create_singlethread_workqueue("stm32_key");
	INIT_WORK(&stm32_key_fcover_work, kpd_stm32_key_work_handler);
	//wake_lock_init(&stm32_key_suspend_lock, WAKE_LOCK_SUSPEND, "stm32_key wakelock");
	wakeup_source_init(&stm32_key_suspend_lock, "stm32_key wakelock");
        ...
        if (node) {
				...
				printk(KERN_ALERT "[stm32_key]ints gpio:xx debounce:%d irq:%d type:%d\n",stm32_keydebounce,stm32_key_irqnr,stm32_key_eint_type);
                ret =
                    request_irq(stm32_key_irqnr, (irq_handler_t) kpd_stm32_key_eint_handler, IRQ_TYPE_EDGE_RISING,
                                "STM32_KEY-eint", NULL);
                if (ret > 0)
                        printk(KERN_ALERT "[stm32_key] EINT IRQ LINENNOT AVAILABLE\n");
                /*enable_irq(g_pmic_irq); */
        } else
                printk(KERN_ALERT "[stm32_key]can't find compatible node\n");
		//enable_irq(296);
	disable_irq_nosync(stm32_key_irqnr);
...
//	printk(KERN_ALERT "[finger_key]---\n");
	...
	wakeup_source_init(&finger_key_suspend_lock, "finger_key wakelock");
        node = of_find_compatible_node(NULL, NULL, "mediatek, FINGER_KEY-eint");
        if (node) {
                of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
				of_property_read_u32_array(node, "interrupts", ints1, ARRAY_SIZE(ints1));
				//rfid_switch_pin = of_get_named_gpio(node, "gpio-rfid-switch-gpio", 0);//拨动开关
...
//				printk(KERN_ALERT "[finger_key]ints gpio:xx debounce:%d irq:%d type:%d\n",finger_keydebounce,finger_key_irqnr,finger_key_eint_type);
  ...
  if (NULL == stm32_dl_proc_entry)
  {
          printk("proc_create %s failed\n", STM32_DL_FW_PROC_NAME);
  }
  wake_stm32_entry = proc_create(WAKE_STM32_PROC_NAME, 0777, NULL, &wake_stm32_proc_fops);
  if (NULL == wake_stm32_entry)
  {
          printk("proc_create %s failed\n", WAKE_STM32_PROC_NAME);
  }
  reset_stm32_entry = proc_create(RESET_STM32_PROC_NAME, 0777, NULL, &reset_stm32_proc_fops);
  if (NULL == reset_stm32_entry)
  {
	...
	aeon_gpio_set("aeon_reset_stm32_low");
#ifdef AEON_CHARGE_STATUS_SUPPORT
  charge_status_entry = proc_create(CHARGE_STATUS_PROC_NAME, 0777, NULL, &charge_status_proc_fops);
  if (NULL == charge_status_entry)
  {
