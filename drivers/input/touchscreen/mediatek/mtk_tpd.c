...
#define TPD_PROC_NAME	"AEON_TPD"
static struct proc_dir_entry *tpd_proc_entry=NULL;
#define TPD_PROC_FIRMWARE	"AEON_TP_FW"
static struct proc_dir_entry *tpd_firmware_entry=NULL;
#endif
...
/* sanford.lin add on 20160308 for get driver information */
#ifdef AEON_DEVICE_PROC_MANAGER
static ssize_t tpd_proc_oem_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
    char *ptr = NULL;
	int len, err = -1;
...
//	else
//		ptr += sprintf(ptr, "unknow tpd name\n");
...
static const struct file_operations tpd_proc_fops = { 
    .read = tpd_proc_oem_read
};

static ssize_t tpd_proc_firmware_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{      
	char *page = NULL;
    char *ptr = NULL;
	int len, err = -1;
...
//	if (g_tpd_drv)
//	{
		if (tpd_firmware_version[1])
			ptr += sprintf(ptr, "0x%x\n", (tpd_firmware_version[0]<<16 | tpd_firmware_version[1]));
		else
			ptr += sprintf(ptr, "0x%x\n", tpd_firmware_version[0]);
//	}
//	else
//		ptr += sprintf(ptr, "unknow tpd firmware\n");
...
static const struct file_operations tpd_proc_firmware_fops = { 
    .read = tpd_proc_firmware_read
};
...
/*Fix build errors,as some projects  cannot support these apis while bring up*/
	...
	/* sanford.lin add on 20160308 for get driver information */
#ifdef AEON_DEVICE_PROC_MANAGER
		tpd_proc_entry = proc_create(TPD_PROC_NAME, 0777, NULL, &tpd_proc_fops);
		if (NULL == tpd_proc_entry)
		{
			printk("proc_create %s failed\n", TPD_PROC_NAME);
		}
		tpd_firmware_entry = proc_create(TPD_PROC_FIRMWARE, 0777, NULL, &tpd_proc_firmware_fops);
		if (NULL == tpd_firmware_entry)
		{
			printk("proc_create %s failed\n", TPD_PROC_FIRMWARE);
		}	 
