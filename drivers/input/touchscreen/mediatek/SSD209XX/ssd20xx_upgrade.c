		...
		struct solomon_fw_group_header *fw_header)
{
	...
	//qzs add for aeon_firmware_version_check
	tpd_firmware_version[0] = dev->fw_version.display_version;
	SOLOMON_WARNNING("qzssolo tpd_firmware_version[0]=0x%x,dev->fw_version.display_version=0x%x\n",tpd_firmware_version[0],dev->fw_version.display_version);
	//qzs add end
...
int solomon_firmware_fw_check(struct solomon_device *dev)      
{
	...
	if ((ptr->byte_cnt)!=0)
	{
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			...
			{
				SOLOMON_WARNNING("1miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
				//goto chk_err;
			}
		}	
	...
	if ((ptr->byte_cnt)!=0)
	{
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("2miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  if ((ptr->byte_cnt)!=0)
 	{
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("3miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				break;
				//cnt++;
			}
		}	
  ...
  if ((ptr->byte_cnt)!=0)
	{
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("4miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  if ((ptr->byte_cnt)!=0)
  {
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("5miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  if ((ptr->byte_cnt)!=0)
	{
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("6miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  {	
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("7miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  if ((ptr->byte_cnt)!=0)
  {
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("8miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
  ...
  SOLOMON_WARNNING("[CHK]section 9 add = 0x%x, len = %d\n",(int)(ptr->address),(ptr->byte_cnt));
	...
	{	
		...
		for (i = 0; i < ptr->byte_cnt; i++) {
			if (ptr->content[i] != verify_data[i]) 
			{
				SOLOMON_WARNNING("9miss match %04d W:0x%02x R:0x%02x!\n",i, ptr->content[i], verify_data[i]);
				cnt++;
			}
		}	
		kfree(verify_data);
