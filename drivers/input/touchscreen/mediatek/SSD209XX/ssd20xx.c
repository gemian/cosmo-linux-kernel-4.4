...
static s32 int_pin_check(struct solomon_device *ftdev, int retry)
{
	...
	SOLOMON_WARNNING("[ISP]int_pin_check = %d  retry=%d \n",ret,retry);
...
int solomon_goto_bios(void)  //HTC_L2052_A01-1765
{
	...
	{	
		...
		//gpio_set_value(misc_dev->reset_pin, 0);
		tpd_gpio_output(tpd_rst_gpio_number, 0);
		//aeon_gpio_set("aeon_tp_solomon_rst0");
		msleep(5);
		//gpio_set_value(misc_dev->reset_pin, 1);
		tpd_gpio_output(tpd_rst_gpio_number, 1);
		//aeon_gpio_set("aeon_tp_solomon_rst1");
		msleep(20);
	}
	return 0;
}
...
static int solomon_hw_init(void)
{
	...
	if (node) {
		...
		TPD_DMESG("Device tpd_intr_type = %d! tpd_int_gpio_number %d\n", tpd_intr_type,tpd_int_gpio_number);
		TPD_DMESG("tpd_intr_type = %d!IRQF_TRIGGER_LOW   ints1[0]   %d\n", tpd_intr_type,ints1[0]);
	} else {
...
static int solomon_fw_update(struct solomon_device *ftdev)
{
	...
	if (1){//(request_firmware(&fw, fw_name, &ftdev->client->dev) != 0) {
		SOLOMON_TIME("H");
