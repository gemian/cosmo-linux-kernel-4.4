...
enum audio_system_gpio_type {
	...
	GPIO_HEADOHONE_EN_HIGH,
	GPIO_HEADOHONE_EN_LOW,
	GPIO_HEADOHONE_CS_HIGH,
	GPIO_HEADOHONE_CS_LOW,	
	GPIO_NUM
};
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_HEADOHONE_EN_HIGH] = {"headphone_en_high", false, NULL},
	[GPIO_HEADOHONE_EN_LOW] = {"headphone_en_low", false, NULL},
	[GPIO_HEADOHONE_CS_HIGH] = {"headphone_cs_high", false, NULL},
	[GPIO_HEADOHONE_CS_LOW] = {"headphone_cs_low", false, NULL},	
};
#endif
...
static DEFINE_MUTEX(gpio_request_mutex);
 #define ENABLE_SPEAKER_PROC_NAME        "AEON_ENABLE_SPEAKER"
static struct proc_dir_entry *enable_speaker_entry;
int enable_speaker_flag = -1;
static ssize_t  ENABLE_SPEAKER_read(struct file *filp, char __user *buffer, size_t size, loff_t *ppos);
static ssize_t  ENABLE_SPEAKER_write(struct file *file, const char *buffer, size_t count,loff_t *data);
 static const struct file_operations enable_speaker_proc_fops = {
         .read  = ENABLE_SPEAKER_read,
         .write = ENABLE_SPEAKER_write
 };
...
#endif
	enable_speaker_entry = proc_create(ENABLE_SPEAKER_PROC_NAME, 0777, NULL, &enable_speaker_proc_fops);
	if (NULL == enable_speaker_entry)
	{
		  printk("proc_create %s failed\n", ENABLE_SPEAKER_PROC_NAME);
	}
...
EXPORT_SYMBOL(AudDrv_GPIO_EXTAMP2_Select);
...
static ssize_t  ENABLE_SPEAKER_write(struct file *file, const char *buffer, size_t count,loff_t *data)
 {
         char Buf[4];
         int stat,ret=0;
         printk("ENABLE_SPEAKER_write\n");
         if (copy_from_user(Buf, buffer, 4)){
                  return -EFAULT;
         }
         ret = sscanf(Buf,"%d",&stat);
         printk("ENABLE_SPEAKER_write stat=%d\n",stat);
		 enable_speaker_flag = stat;
         if(stat == 1){
...
int AudDrv_GPIO_headphone_en(int bEnable)
{
	...
	if (bEnable == 1){
		...
		AudDrv_GPIO_Select(GPIO_HEADOHONE_EN_LOW);
	}

	return retval;
}
EXPORT_SYMBOL(AudDrv_GPIO_headphone_en);
int AudDrv_GPIO_headphone_cs(int bEnable)
{
	int retval = 0;

	if (bEnable == 1){
		AudDrv_GPIO_Select(GPIO_HEADOHONE_CS_HIGH);
	}
	else{
